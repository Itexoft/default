...
internal sealed partial class SysTimerAndroid(int millesecondsTimeout, bool callIfCancelled, Action source)
    : SysTimer(millesecondsTimeout, callIfCancelled, source)
{
    ...
    private protected override unsafe void StartInternal()
    {
        ...
        *(nint*)(sev + unionOffset + nint.Size) = nint.Zero;
        ...
        if (Native.timer_settime(this.timerId, 0, in spec, nint.Zero) != 0)
        {
            Invoke(this.handle, true);
    ...
    private protected override void DisposeInternal()
    {
        ...
        Native.timer_delete(this.timerId);
    }
    ...
    {
        [LibraryImport("libc", EntryPoint = "timer_create", SetLastError = true)]
        internal static partial int timer_create(int clockid, nint sevp, out nint timerid);

        [LibraryImport("libc", EntryPoint = "timer_settime", SetLastError = true)]
        internal static partial int timer_settime(nint timerid, int flags, in Itimerspec newValue, nint oldValue);

        [LibraryImport("libc", EntryPoint = "timer_delete", SetLastError = true)]
