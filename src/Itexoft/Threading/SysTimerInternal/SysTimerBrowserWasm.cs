    ...
    : SysTimer(millesecondsTimeout, callIfCancelled, source)
{
    ...
    private protected override void StartInternal() => this.timeoutId = Native.SetTimeout(this.TimeoutAction, this.millesecondsTimeout);

    private void TimeoutAction() => Invoke(this.handle, false);

    private protected override void DisposeInternal() => Native.ClearTimeout(this.timeoutId);
