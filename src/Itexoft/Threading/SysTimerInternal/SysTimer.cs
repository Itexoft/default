...
internal abstract class SysTimer
{
    ...
    public bool IsFinished => this.timedOut;
    ...
    public bool Cancel() => this.Terminate(true);
    ...
    private bool Terminate(bool cancelled)
    {
    ...
    protected static void Invoke(nuint handle, bool cancelled)
    {
        if (memory.TryRef(handle, out var timer))
            timer.Value.Terminate(cancelled);
    }
}
