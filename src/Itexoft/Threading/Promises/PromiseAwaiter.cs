...
public readonly struct PromiseAwaiter<TResult> : ICriticalNotifyCompletion
{
    ...
    internal static PromiseAwaiter<TResult> UncompletedFunc(Func<TResult> func, bool promise) =>
        new(PromiseAwaiter.FromState(new State<TResult>(func.Required(), promise)));
...
file class State
{
    ...
    public bool TryComplete(Exception? exception = null)
    {
        ...
        if (this.state is SysTimer timer)
            timer.Cancel();
