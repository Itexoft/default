...
public struct Deferred<TResult>(Func<TResult> factory)
{
    ...
    public bool TryGetValueIfCreated([NotNullWhen(true)] out TResult? value)
    {
        if (this.latch)
        {
            value = this.value!;
...
[method: MethodImpl(MethodImplOptions.AggressiveInlining)]
public struct DeferredCtor<TResult>() where TResult : class, new()
{
    private Latch latch = new();
    private TResult value = null!;
    private ExceptionDispatchInfo? exceptionDispatchInfo;
