...
public struct InvokeGatePulse<TContext, TResult>
{
    ...
    internal InvokeGatePulse(Func<TContext?, TResult> callback, TContext? pendingContext, int intervalMilliseconds)
    {
        ...
        this.intervalTicks = TimeSpan.FromMilliseconds(intervalMilliseconds.RequiredPositiveOrZero()).Ticks;
    ...
    public bool TryTrailing(in TContext? context)
    {
        ...
        var due = Atomic.Read(ref this.timestamp);
        ...
        if (Atomic.Read(ref this.timestamp) == 0)
    ...
    public bool TryDebounce(in TContext? context)
    {
        ...
        Atomic.Write(ref this.timestamp, TimeUtils.CachedTimestampTicks + this.intervalTicks);
    ...
    private bool TryWritePending(in TContext? context)
    {
        ...
        {
            Atomic.Write(ref this.pendingLock, 0);
        }
    }
