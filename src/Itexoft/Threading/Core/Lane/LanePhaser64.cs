...
public sealed class LanePhaser64
{
    ...
    public int ReadPhase() => Atomic.Read(ref this.phase);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public ulong ReadArrivedMask() => Atomic.Read(ref this.arrivedMask);
    ...
    {
        var myPhase = Atomic.Read(ref this.phase);
        ...
        while (Atomic.Read(ref this.phase) == myPhase)
            sw.SpinOnce();
    ...
    public void AwaitAdvance(int observedPhase)
    {
        ...
        while (Atomic.Read(ref this.phase) == observedPhase)
            sw.SpinOnce();
    }
}
