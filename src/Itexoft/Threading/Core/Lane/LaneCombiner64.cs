...
public sealed class LaneCombiner64<TState, TRequest, TResult, TOp> where TOp : struct, ILaneOp64<TState, TRequest, TResult>
{
    private const int combinerFree = 0;
    private const int combinerBusy = 1;

    private readonly TRequest[] requests;
    private readonly TResult[] results;
    ...
    public LaneCombiner64(TState state)
    {
        this.state = state;

        this.requests = new TRequest[64];
        this.results = new TResult[64];
    }
    ...
    private bool TryPublish(in Lane64 lane, in TRequest request)
    {
        ...
        UnsafeArray.AtUnchecked(this.requests, lane.Index) = request;
    ...
    private TResult Consume(in Lane64 lane)
    {
        ...
        ref var slot = ref UnsafeArray.AtUnchecked(this.results, lane.Index);
        var result = slot;
    ...
    private bool DrainBatch()
    {
        ...
        while (batch != 0)
        {
            ...
            ref var requestSlot = ref UnsafeArray.AtUnchecked(this.requests, index);

            TOp.Invoke(ref this.state, in requestSlot, out var result);

            requestSlot = default!;

            UnsafeArray.AtUnchecked(this.results, index) = result;
