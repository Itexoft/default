...
public sealed class LaneCombiner64<TState, TRequest, TResult, TOp>(TState state) where TOp : struct, ILaneOp64<TState, TRequest, TResult>
{
    private const int combinerFree = 0;
    private const int combinerBusy = 1;

    private readonly TRequest[] requests = new TRequest[64];
    private readonly TResult[] results = new TResult[64];
