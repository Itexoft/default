...
public struct Epoch64
{
    ...
    public ulong Epoch0Mask
    {
        ...
        get => Atomic.Read(ref this.epoch0Mask);
    }

    public ulong Epoch1Mask
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Atomic.Read(ref this.epoch1Mask);
    }

    public int Epoch
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        get => Atomic.Read(ref this.epoch);
    }
    ...
    public int Enter(in Lane64 lane)
    {
        ...
        while (true)
        {
            var e = Atomic.Read(ref this.epoch);
            ...
            if (Atomic.Read(ref this.epoch) == e)
                return e;
    ...
    public int AdvanceAndWait()
    {
        ...
        if ((old & 1) == 0)
        {
            while (Atomic.Read(ref this.epoch0Mask) != 0)
                sw.SpinOnce();

            return old + 1;
        }

        while (Atomic.Read(ref this.epoch1Mask) != 0)
            sw.SpinOnce();
