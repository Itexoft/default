...
public static class LatchExtensions
{
    extension(ref Latch latch)
    {
        ...
        public void TryWait(CancelToken cancelToken = default)
        {
            for (var i = 0; !latch.Try() && !cancelToken.IsRequested;)
                Spin.Wait(ref i);
        ...
        {
            for (var i = 0; !latch && !cancelToken.IsRequested;)
                Spin.Wait(ref i);

            cancelToken.ThrowIf();
        }

        public void SleepWait(CancelToken cancelToken = default)
        {
            for (var i = 0; i < 1024 && !latch && !cancelToken.IsRequested; i++)
                Thread.Yield();

            while (!latch && !cancelToken.IsRequested)
                Thread.Sleep(1);
