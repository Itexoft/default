...
public sealed class CosineVectorIndex(int dimension)
{
    ...
    public string? Select(float[] query)
    {
    ...
    public IEnumerable<string> Rank(float[] query)
    {
        var snapshot = this.PrepareQuery(query, out var queryNorm);
    ...
    public IEnumerable<string> Rank(float[] query, int topK, bool includeExactMatch = false)
    {
        ...
        if (topK == 1)
        {
            if (TrySelectSingle(snapshot, query, queryNorm, includeExactMatch, out var selected))
        ...
        for (var cursor = snapshot; cursor is not null; cursor = cursor.next)
        {
            ...
            var item = (cursor.value, VectorMath.Dot(query, cursor.vector.Span) / (queryNorm * cursor.norm));
    ...
    private void InsertInternal(ReadOnlyMemory<float> vector, string value)
    {
        ...
        var norm = VectorMath.NormL2(vector.Span, nameof(vector));
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private Node? PrepareQuery(float[] query, out float queryNorm)
    {
        ...
        queryNorm = VectorMath.NormL2(query, nameof(query));
