...
public sealed class SysFile : IFileSystem
{
    ...
    public IStreamRws<byte> Open(string path, SysFileMode mode) =>
        new FileStream(RequirePath(path), GetMode(mode), GetAccess(mode), FileShare.None, defaultBufferSize, FileOptions.None).AsSstreamRws();
    ...
    public IStreamRws<byte> Create(string path, FileAttributes attributes = FileAttributes.Normal, FileOptions options = FileOptions.None)
    {
        var fullPath = RequirePath(path);
