...
public struct CharStreamBrws(IStreamRws<byte> stream, Encoding encoding) : IDisposable
{
    private readonly IStreamRws<byte> stream = stream.Required();
    ...
    public void Seek(long index)
    {
        ...
        for (long remaining = index; remaining > 0;)
        {
