...
public sealed class RewindStream<T>(IStreamR<T> inner, int initialCapacity = 256, bool leaveOpen = false) : IStreamR<T> where T : unmanaged
{
    private readonly int initialCapacity = initialCapacity.RequiredPositiveOrZero();
    private readonly IStreamR<T> inner = inner.Required();
    private T[] buffer = [];
    ...
    public int Read(Span<T> buffer, CancelToken cancelToken = default)
    {
        ...
        try
        {
            return this.ReadCore(buffer, cancelToken);
        }
        finally
        {
    ...
    private int ReadCore(Span<T> buffer, CancelToken cancelToken = default)
    {
        if (buffer.Length == 0)
            return 0;
        ...
        var read = this.inner.Read(buffer[readFromBuffer..], cancelToken);

        if (read <= 0)
            return readFromBuffer == 0 ? read : readFromBuffer;

        if (this.IsBuffering)
            this.Append(buffer.Slice(readFromBuffer, read));
    ...
    private int TryReadBuffered(Span<T> destination)
    {
        ...
        this.buffer.AsSpan(offset, toCopy).CopyTo(destination);
        this.Position += toCopy;

        return toCopy;
    }

    private void Append(ReadOnlySpan<T> data)
    {
        if (data.Length == 0)
            return;
    ...
    private void EnsureCapacity(int additional)
    {
        if (this.buffer.Length == 0)
        {
            ...
            this.buffer = ArrayPool<T>.Shared.Rent(size);
