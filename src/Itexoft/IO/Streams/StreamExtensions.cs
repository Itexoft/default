...
{
    private static int ReadAtLeastCore<T>(IStreamR<T> stream, Span<T> buffer, int minimumBytes, CancelToken cancelToken) where T : unmanaged
    {
        var totalRead = 0;
    ...
    extension<T>(IStreamR<T> stream) where T : unmanaged
    {
        public void CopyTo(IStreamW<T> target, int bufferSize = 8192)
        {
            ...
            for (Span<T> one = stackalloc T[bufferSize.RequiredPositive()];;)
            {
    ...
    extension<T>(IStreamR<T> stream) where T : unmanaged
    {
        ...
        {
            if (stream is IStreamRl<T> streamRl)
            {
                Memory<T> memory = new T[streamRl.Length];

                for (var r = 0 - 1; r < memory.Length;)
