...
internal static class SuperblockInspector
{
    ...
    /// </summary>
    /// <param name="stream">System.IO.Stream to inspect. The method temporarily rewinds it to the beginning and restores the original position.</param>
    ...
    public static void Inspect(System.IO.Stream stream, out SuperblockLayout.SuperblockState state, out int pageSize, out bool hasValidSuperblock)
    {
        stream.Required();

        if (!stream.CanSeek)
            throw new ArgumentException("System.IO.Stream must support seeking.", nameof(stream));
        ...
        try
        {
            foreach (var candidateSize in EnumerateCandidatePageSizes())
            {
                ...
                if (stream.Length >= candidateSize * 2 && TryReadPage(stream, candidateSize, candidateSize, out var buffer1))
    ...
    private static bool TryReadPage(System.IO.Stream stream, long offset, int length, [NotNullWhen(true)] out byte[]? buffer)
    {
        var originalPosition = stream.Position;
        buffer = null;
