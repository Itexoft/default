...
internal sealed class JournalManager : IDisposable
{
    ...
    /// </summary>
    /// <param name="journalStream">System.IO.Stream used to persist journal entries.</param>
    public JournalManager(System.IO.Stream journalStream)
    {
        journalStream.Required();

        if (!journalStream.CanRead || !journalStream.CanWrite || !journalStream.CanSeek)
            throw new ArgumentException("System.IO.Stream must support read, write, and seek.", nameof(journalStream));
