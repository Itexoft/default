...
public static class EmbeddedWebServer
{
        ...
        CancelToken cancelToken = default)
    {
        ...
        var server = new NetHttpServer(endpoint, (request, ct) => ServeRequest(request, bundle, options, rpc, ct));

        server.Start(cancelToken);

        return new EmbeddedWebHandle(bundleId, server);
    }
