...
1. **Build-time tooling** (`src/Itexoft/EmbeddedWeb/EmbeddedWebAssets.targets`) that gathers static files and embeds
   them as resources. Import `src/Itexoft/EmbeddedWeb/Itexoft.targets` if you reference the sources directly.
...
2. Embeds them directly into the assembly as `EmbeddedResource` entries named `EmbeddedWeb.{BundleId}/{relativePath}`.
