...
internal static partial class DkonSerializer
{
    ...
    private static string ParseMultilineValue(ref Reader reader)
    {
        ...
        while (reader.Index < source.Length)
        {
            ...
            if (IsStandaloneMarkerLine(source, lineStart, lineEnd, DkonFormat.MarkerMultilineClose.AsSpan()))
            {
            ...
            if (eolLength > 0)
            {
                ...
                if (nextLineStart < source.Length)
                {
                    ...
                    if (IsStandaloneMarkerLine(source, nextLineStart, nextLineEnd, DkonFormat.MarkerMultilineClose.AsSpan()))
    ...
    private static void WriteNodeScalar(in DkonNode node, bool hasAlt, bool hasRef, ref ArrayBufferWriter<char> writer)
    {
        ...
        {
            writer.WriteMarkerMultilineOpen();
            writer.WriteLineFeed();
    ...
    private static bool TryConsumeEscapedMarker(ref Reader reader, ReadOnlySpan<char> marker, ref ArrayBufferWriter<char> buffer)
    {
        ...
        var p = index;
        var coalesced = reader.ReusableColon && marker[0] == DkonFormat.Colon && index > 0 && source[index - 1] == DkonFormat.Colon;
    ...
    private static bool TryMatchMarker(ref Reader reader, ReadOnlySpan<char> marker)
    {
        ...
        if (reader.ReusableColon && marker[0] == DkonFormat.Colon)
        {
            if (index == 0 || source[index - 1] != DkonFormat.Colon)
                return false;
    ...
    private static bool TryConsumeMarker(ref Reader reader, ReadOnlySpan<char> marker)
    {
        ...
        if (reader.ReusableColon && marker.Length > 0 && marker[0] == DkonFormat.Colon)
    ...
    private static bool IsStandaloneMarkerLine(ReadOnlySpan<char> source, int lineStart, int lineEnd, ReadOnlySpan<char> marker)
    {
        var i = lineStart;
