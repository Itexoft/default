...
{
    public T? Deserialize<T>(string dkon) => (T?)this.Deserialize(dkon, typeof(T));

    public object? Deserialize(string dkon, Type type)
    {
        if (type == typeof(string))
        {
            ...
            if (root.IsEnding)
                return (object)root.ToString();

            throw new FormatException($"Cannot convert DKON value '{dkon}' to 'string'.");
        }
        else
        {
            if (string.IsNullOrWhiteSpace(dkon))
            ...
            var root = DeserializeNodeOrEmpty(dkon);
            var materialized = new Dictionary<DkonNode, object?>(ReferenceEqualityComparer.Instance);
            var value = this.ReadNodeAsValue(root, typeInfo, true, materialized);

            return value;
        }
    }

    private object? ReadNodeAsValue(DkonNode node, JsonTypeInfo typeInfo, bool isRoot, Dictionary<DkonNode, object?> materialized)
    {
        switch (typeInfo.Kind)
        {
            case JsonTypeInfoKind.Object:
                return this.ReadObjectAsValue(node, typeInfo, isRoot, materialized);
            case JsonTypeInfoKind.Dictionary:
                return this.ReadDictionaryAsValue(node, typeInfo, isRoot, materialized);
            case JsonTypeInfoKind.Enumerable:
                return this.ReadEnumerableAsValue(node, typeInfo, isRoot, materialized);
            default:
                return this.ReadScalarAsValue(node, typeInfo, isRoot);
        }
    }

    private object? ReadObjectAsValue(DkonNode node, JsonTypeInfo typeInfo, bool isRoot, Dictionary<DkonNode, object?> materialized)
    {
        if (materialized.TryGetValue(node, out var existing))
            return existing;

        var instance = CreateObjectInstance(typeInfo.CreateObject, typeInfo.Type);

        if (instance is null)
            return DefaultForType(typeInfo.Type);

        materialized[node] = instance;
        ...
        for (var current = first; current is not null; current = current.Next)
        {
            ...
            if (this.TryGetDkonTypeInfo(property.PropertyType, out var valueTypeInfo))
                value = this.ReadNodeAsValue(current.Ref, valueTypeInfo, false, materialized);
            else
    ...
    private object? ReadDictionaryAsValue(DkonNode node, JsonTypeInfo typeInfo, bool isRoot, Dictionary<DkonNode, object?> materialized)
    {
        if (materialized.TryGetValue(node, out var existing))
            return existing;
        ...
        var instance = CreateDictionaryInstance(typeInfo.CreateObject, typeInfo.Type);
    ...
    private object? ReadEnumerableAsValue(DkonNode node, JsonTypeInfo typeInfo, bool isRoot, Dictionary<DkonNode, object?> materialized)
    {
        if (materialized.TryGetValue(node, out var existing))
            return existing;
        ...
        if (typeInfo.Type.IsArray)
            return this.ReadArrayAsValue(node, first, elementType, hasTypedElementInfo, elementTypeInfo, materialized);

        var collection = CreateCollectionInstance(typeInfo.CreateObject, typeInfo.Type);

        if (collection is null)
            return DefaultForType(typeInfo.Type);

        materialized[node] = collection;
        ...
        for (var current = first; current is not null; current = current.Next)
        {
            ...
            if (hasTypedElementInfo)
                value = this.ReadNodeAsValue(current, elementTypeInfo, false, materialized);
    ...
    private object? ReadArrayAsValue(
        DkonNode node,
        DkonNode? first,
        Type elementType,
        bool hasTypedElementInfo,
        JsonTypeInfo? elementTypeInfo,
        Dictionary<DkonNode, object?> materialized)
    {
        if (materialized.TryGetValue(node, out var existing))
            return existing;
        ...
        for (var current = first; current is not null; current = current.Next)
        {
            ...
            array.SetValue(CoerceToTargetOrDefault(value, elementType), index++);
        }
    ...
    public string Serialize(object obj, Type type, bool beautify = false) => this.Serialize(obj, type, out _, beautify);

    internal string Serialize(object obj, Type type, out DkonSyntaxLevel syntaxLevel, bool beautify)
    {
        if (obj is null)
        {
            syntaxLevel = DkonSyntaxLevel.None;
    ...
    private object? ReadArrayAsValue(DkonNode? first, Type elementType, bool hasTypedElementInfo, JsonTypeInfo? elementTypeInfo)
    {
        ...
        for (var current = first; current is not null; current = current.Next)
        {
            ...
            items.Add(CoerceToTargetOrDefault(value, elementType));
        }
    ...
    private object? ReadTextAsType(string raw, Type targetType)
    {
        ...
        {
            if (Enum.TryParse(scalarType, raw, true, out var enumValue) && Enum.IsDefined(scalarType, enumValue))
                return enumValue;

            throw new FormatException($"Cannot convert DKON enum value '{raw}' to '{targetType}'.");
        }
    ...
    private DkonNode? WriteValueAsNode<T>(T value, Type declaredType, bool isRoot)
    {
        ...
        {
            if (this.TryWriteDynamicValueAsNode(value, runtimeType, isRoot, out var dynamicNode))
                return dynamicNode;
    ...
    private bool TryWriteDynamicValueAsNode<T>(T value, Type runtimeType, bool isRoot, out DkonNode? node)
    {
        if (value is string)
        {
            node = WriteScalarAsNode(value);
        ...
        {
            node = this.WriteDynamicDictionaryAsNode(dictionary, isRoot);
        ...
        {
            node = this.WriteDynamicEnumerableAsNode(enumerable, isRoot);

            return true;
        }

        if (IsDynamicScalarType(runtimeType))
        {
            node = WriteScalarAsNode(value);
    ...
    private DkonNode? WriteDynamicDictionaryAsNode(IDictionary dictionary, bool isRoot)
    {
        DkonNode? head = null;
        DkonNode? tail = null;

        foreach (DictionaryEntry entry in dictionary)
            AppendDynamicDictionaryItem(ref head, ref tail, entry.Key, entry.Value, this);
    ...
    private DkonNode? WriteDynamicEnumerableAsNode(IEnumerable enumerable, bool isRoot)
    {
        DkonNode? head = null;
        DkonNode? tail = null;
        ...
        foreach (var item in enumerable)
        {
            var itemNode = this.WriteValueAsNode(item, item?.GetType() ?? typeof(object), false);
    ...
    private static void AppendDynamicDictionaryItem(ref DkonNode? head, ref DkonNode? tail, object? key, object? value, DkonFormat context)
    {
        var keyText = KeyToText(key);
        ...
        var valueNode = context.WriteValueAsNode(value, value?.GetType() ?? typeof(object), false);
    ...
    private static T CreateObjectInstance<T>(Func<T>? factory, Type declaredType)
    {
        if (factory is not null)
            return factory();

        var instance = CreateMissingFactoryInstance(declaredType);

        return instance is null ? default! : (T)instance;
    }

    private static IDictionary? CreateDictionaryInstance(Func<object>? factory, Type declaredType)
    {
        var instance = CreateObjectInstance(factory, declaredType);

        return instance as IDictionary;
    }

    private static IList? CreateCollectionInstance(Func<object>? factory, Type declaredType)
    {
        var instance = CreateObjectInstance(factory, declaredType);
    ...
    private static object? CreateMissingFactoryInstance(Type declaredType)
    {
    ...
    private static bool IsDynamicScalarType(Type type)
    {
