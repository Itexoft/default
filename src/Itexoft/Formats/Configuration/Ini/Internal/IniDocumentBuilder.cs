...
internal sealed class IniDocumentBuilder(string source, IniReaderOptions options)
{
    ...
    public IniSectionBuilder Global { get; } = new(string.Empty, ReadOnlyMemory<char>.Empty, true, 0, options.KeyComparer);

    public ReadOnlyMemory<char> Slice(int start, int length) =>
        length <= 0 ? ReadOnlyMemory<char>.Empty : source.AsMemory(start, length);

    public IniSectionBuilder GetOrCreateSection(ReadOnlyMemory<char> nameMemory, int lineNumber)
    {
        ...
        if (this.sectionIndex.TryGetValue(nameText, out var existing))
        {
            ...
            var duplicate = new IniSectionBuilder(nameText, nameMemory, false, lineNumber, options.KeyComparer);
            this.sections.Add(duplicate);

            return duplicate;
        }

        var section = new IniSectionBuilder(nameText, nameMemory, false, lineNumber, options.KeyComparer);
