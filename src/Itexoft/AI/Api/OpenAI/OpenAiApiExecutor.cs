...
internal sealed class OpenAiApiExecutor
{
    ...
    private readonly DeferredPool<NetHttpClient> clientPool;
    private readonly TimeSpan requestTimeout;

    internal OpenAiApiExecutor(DeferredPool<NetHttpClient> clientPool, string basePath, TimeSpan requestTimeout, RetryPolicy retryPolicy)
    {
    ...
    public OpenAiApiExecutor WithRetryPolicy(RetryPolicy retryPolicy) =>
        new(this.clientPool, this.basePath, this.requestTimeout, retryPolicy);
    ...
    public TResponse PostJson<TResponse>(string path, object payload, CancelToken cancelToken = default)
    {
        ...
        using (this.clientPool.GetValue(out var client))
            return this.RetryPolicy.Run<TResponse>(token => this.PostJsonCore<TResponse>(client, path, payloadBytes, token), cancelToken);
    }
        ...
        using (this.clientPool.GetValue(out var client))
        {
            var response = this.RetryPolicy.Run(
                cancelToken =>
                {
                    try
                    {
                        var response = this.PostRaw(client, path, payloadBytes, cancelToken);
    ...
    private TResponse PostJsonCore<TResponse>(NetHttpClient client, string path, byte[] payload, CancelToken cancelToken)
    {
        try
        {
            var response = this.PostRaw(client, path, payload, cancelToken);
    ...
    private NetHttpResponse PostRaw(NetHttpClient client, string path, byte[] payload, CancelToken cancelToken)
    {
        ...
        return client.Send(request, cancelToken);
    }
