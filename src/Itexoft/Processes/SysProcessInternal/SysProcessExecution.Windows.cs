...
internal sealed class WindowsSysProcessExecution : SysProcessExecution
{
    ...
    private static SysProcessExecution StartProcess(SysProcessStartRequest request, SafeFileHandle? primaryToken = null)
    {
        ...
        try
        {
            ...
            environmentBlock = runAs
                ? BuildRunAsEnvironmentBlock(primaryToken!, request.Environment)
            ...
            var processInfo = runAs
                ? StartWindowsRunAsProcess(
                    ...
                    anyRedirect,
                    ref startupInfo)
                : StartWindowsCurrentUserProcess(request.ExecutablePath, commandLine, creationFlags, environmentBlock, currentDirectory, anyRedirect, ref startupInfo);
