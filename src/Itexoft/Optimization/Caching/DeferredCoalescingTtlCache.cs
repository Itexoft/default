...
{
    private readonly AtomicDictionary<TKey, CacheEntry> cache = new();
    private readonly DeferredCoalescingCache<TKey, TValue> inFlight = new();

    public void Clear() => this.cache.Clear();

    public TValue GetOrAdd(TKey key, Func<TKey, CancelToken, CacheValue> valueFactory, CancelToken cancelToken = default)
    {
        if (key is null)
        ...
        return this.inFlight.GetOrAddAsync(
            ...
            (k, ct) =>
            {
                var produced = valueFactory(k, ct);
                ...
                if (produced.Ttl > TimeSpan.Zero)
                {
                    ...
                    this.cache.AddOrUpdate(k, entry, (_, existing) => entry.ExpiresAt >= existing.ExpiresAt ? entry : existing);
                }
    ...
    {
        if (this.cache.TryGet(key, out var entry))
        {
