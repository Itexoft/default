...
file unsafe sealed class AtomicDictionaryTest<TKey, TValue> where TKey : unmanaged where TValue : unmanaged
{
    ...
    public AtomicDictionaryTest() : this(default) { }

    private AtomicDictionaryTest(FastConcurrentDictionaryOptions options = default)
    {
    ...
    public int Count
    {
        get
        {
            ...
            for (var i = 0; i < this.shards.Length; i++)
            {
                ...
                total += Atomic.Read(ref ShardCountRef(ref shard));
            }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static Table* ReadTablePointer(ref nint location) => (Table*)Atomic.Read(ref location);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private static void WriteTablePointer(ref nint location, Table* value) =>
        Atomic.Write(ref location, (nint)value);
        ...
        for (var group = 0; group < table->MaxProbeGroups; group++)
        {
            if ((groupWidth == 16 || groupWidth == 8) && baseIndex + groupWidth <= capacity)
            {
                ...
                while (matchMask != 0)
                {
                    ...
                    if (Atomic.Read(ref CtrlAt(table, index)) == h2)
                    {
                ...
                while (emptyMask != 0)
                {
                    ...
                    if (Atomic.Read(ref CtrlAt(table, index)) == ctrlEmpty)
                    {
                        value = default;
            ...
            for (var offset = 0; offset < groupWidth; offset++)
            {
                ...
                var ctrl = Atomic.Read(ref CtrlAt(table, index));
    ...
    private static void InsertAt(Table* table, int index, in TKey key, in TValue value, byte h2)
    {
        ...
        entry.Key = key;
        entry.Value = value;
        Atomic.Write(ref CtrlAt(table, index), h2);
    }
    ...
    private bool TryAddLocked(in TKey key, in TValue value, ulong hash, byte h2, ref Shard shard)
    {
        while (true)
        {
            ...
            if (table == null)
                throw new InvalidOperationException(nameof(AtomicDictionaryTest<TKey, TValue>));
    ...
    private bool TryRemoveLocked(in TKey key, ulong hash, byte h2, ref Shard shard, out TValue value)
    {
        ...
        {
            Atomic.Write(ref CtrlAt(table, index), ctrlEmpty);
            ...
            while (true)
            {
                ...
                Atomic.Write(ref CtrlAt(table, prev), ctrlEmpty);
                ShardTombstonesRef(ref shard)--;
                current = prev;
            }
        }
        ...
        {
            Atomic.Write(ref CtrlAt(table, index), ctrlDeleted);
            ShardTombstonesRef(ref shard)++;
        }
    ...
    private long GetMinActiveEpoch()
    {
        ...
        {
            var epoch = Atomic.Read(ref this.SlotAt(i));
    ...
    private int TryAcquireSlot()
    {
        ...
        {
            var word = Atomic.Read(ref this.slotWords[wordIndex]);
    ...
    private int GetThreadSlotIndex()
    {
        ...
        for (var probe = 0; probe <= mask; probe++)
        {
            ...
            var state = Atomic.Read(ref cache);
            ...
            if (slotIndex >= 0)
            {
                Atomic.Write(ref cache, ((long)threadId << 32) | (uint)slotIndex);

                return slotIndex;
            }

            Atomic.Write(ref cache, 0);
    ...
    private void ReleaseSlot(int slotIndex)
    {
        ...
        if (this.qsbr.Slots != null)
            Atomic.Write(ref this.SlotAt(slotIndex), 0);
    ...
    private void EnterRead(int slotIndex)
    {
        ...
        var epoch = Atomic.Read(ref this.qsbr.GlobalEpoch);
        Atomic.Write(ref this.SlotAt(slotIndex), epoch);
    }
    ...
    private static void ReleaseBit(ref long word, int bitIndex, object slowLock, ref int slowWaiter)
    {
        ...
        if (Atomic.Read(ref slowWaiter) > 0)
        {
            lock (slowLock)
        ...
        for (var wordIndex = 0; wordIndex < readyMasks.Length; wordIndex++)
        {
            ...
            while (true)
            {
                ...
                while (mask != 0)
                {
                    ...
                    if (Atomic.Read(ref slot.State) == combineReady
                        && Interlocked.CompareExchange(ref slot.State, combineProcessing, combineReady) == combineReady)
                    {
                        ...
                        Atomic.Write(ref slot.State, combineDone);
                    }
    ...
    private static int TryPublishCombineRequest(ref Shard shard, ref CombineRequest request, int start)
    {
        ...
        for (var i = 0; i < slots.Length; i++)
        {
            ...
            if (Atomic.Read(ref slot.State) != combineFree)
                continue;
            ...
            Atomic.Write(ref slot.State, combineReady);
    ...
    private static void WaitForCombineDone(ref CombineRequest slot)
    {
        ...
        while (Atomic.Read(ref slot.State) != combineDone)
        {
            sw.SpinOnce();
    ...
    private static bool WaitForCombineBool(ref CombineRequest slot)
    {
        ...
        Atomic.Write(ref slot.State, combineFree);
