...
public sealed class TerminalExplorerButton
{
    ...
    internal TerminalExplorerButtonResult Invoke(TerminalObjectExplorer explorer, TerminalExplorerSession session) => this.handler(explorer, session);

    internal bool IsVisible(TerminalObjectExplorer explorer, TerminalExplorerSession session) => this.isVisible?.Invoke(explorer, session) ?? true;
}
