...
using Itexoft.UI.Cli.TerminalKit.Dsl;
using Itexoft.UI.Cli.TerminalKit.Interaction;
using Itexoft.UI.Cli.TerminalKit.Presets;
using Itexoft.UI.Cli.TerminalKit.Reflection;
using Itexoft.UI.Cli.TerminalKit.Rendering;

namespace Itexoft.UI.Cli.TerminalKit.ObjectExplorer;
...
internal sealed class TerminalExplorerSession
{
    ...
    private void BeginPropertyLoad(TerminalExplorerFrame frame, PropertyInfo property, int version, TerminalDispatcher? dispatcher)
    {
        ...
        this.StartBackgroundLoad(async token =>
        {
            try
            {
                ...
                {
                    var value = await Promise.Run(() => property.GetValue(target));
                    var resolution = await this.ResolvePropertyValueAsync(property, value, token);
    ...
    private async Promise<PropertyValueResolution> ResolvePropertyValueAsync(PropertyInfo property, object? rawValue, CancelToken cancelToken)
    {
    ...
    {
        if (returnType == typeof(void) || returnType == typeof(Task) || returnType == typeof(Promise))
            return false;
    ...
    private void HandleMethod(object? target, MethodInfo method)
    {
        ...
        try
        {
            ...
            this.statusMessage = hasReturnValue
                ? $"{method.Name}: {TerminalExplorerValueFormatter.Format(resolvedResult)}"
