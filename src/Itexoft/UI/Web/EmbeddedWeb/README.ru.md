...
Исходники находятся в `src/Itexoft/EmbeddedWeb/`. Компонент обслуживает фронтенд‑бандлы из embedded‑ресурсов и
поднимает легкий HTTP‑пайплайн без раскладки файлов на диск.
...
1. **Слой ресурсов сборки**: статические файлы встраиваются как `EmbeddedResource` с именами вида
...
Опишите бандлы в `.csproj` напрямую как embedded‑ресурсы с каноническим `LogicalName`:
...
Во время `Build`/`Publish` MSBuild встраивает эти файлы в сборку ровно с указанными именами. `EmbeddedWebServer`
считает членами бандла все ресурсы с префиксом `EmbeddedWeb.{BundleId}/{relativePath}`, поэтому `LogicalName` должен
следовать этому контракту.
