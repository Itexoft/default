...
public sealed class NetHttpFormData : IEnumerable<NetHttpFormField>
{
    ...
    public string ToString(NetHttpFormEncoding encoding = NetHttpFormEncoding.FormUrlEncoded)
    {
        if (this.fields.Count == 0)
            return string.Empty;
        var writer = new DynamicMemory<byte>(EstimateLength(this.fields));

        try
        {
            this.WriteTo(ref writer, encoding);
            
            return this.encoding.GetString(writer.AsSpan());
        }
        finally
        {
            writer.Dispose();
    ...
    public void WriteTo(ref DynamicMemory<byte> writer, NetHttpFormEncoding encoding = NetHttpFormEncoding.FormUrlEncoded)
    {
        if (this.fields.Count == 0)
            return;
        ...
        for (var i = 0; i < this.fields.Count; i++)
        {
            ...
            WriteEncoded(ref writer, field.Name.AsSpan(), spaceAsPlus);
            writer.Write((byte)'=');
            WriteEncoded(ref writer, field.Value.AsSpan(), spaceAsPlus);
        }
    }

    public override string ToString() => this.ToString(NetHttpFormEncoding.FormUrlEncoded);
    ...
    private static void WriteEncoded(ref DynamicMemory<byte> writer, ReadOnlySpan<char> text, bool spaceAsPlus)
    {
