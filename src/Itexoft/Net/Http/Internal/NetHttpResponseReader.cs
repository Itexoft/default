...
internal static class NetHttpResponseReader
{
    ...
    public static NetHttpResponse Read(INetStream stream, NetHttpRequest request, Action disconnect, CancelToken cancelToken)
    {
        ...
        if (statusLine.Length == 0)
            throw new NetHttpConnectionClosedException("HTTP response is missing status line.");
...
internal sealed class NetHttpConnectionClosedException(string message) : IOException(message);
