...
public readonly record struct NetHttpPathQuery
{
    ...
    public override string ToString()
    {
        ...
        var writer = new DynamicMemory<byte>(Math.Max(this.Path.Length + 16, 64));

        try
        {
            this.WriteTo(ref writer);

            return Encoding.UTF8.GetString(writer.AsSpan());
        }
        finally
        {
            writer.Dispose();
    ...
    internal void WriteTo(ref DynamicMemory<byte> writer)
    {
        var path = this.Path;

        if (string.IsNullOrEmpty(path))
            WriteByte(ref writer, (byte)'/');
        else
            WriteUtf8(ref writer, path.AsSpan());
        ...
        for (var i = 0; i < this.Query.Count; i++)
        {
            ...
            WriteEncoded(ref writer, param.Name.AsSpan());
            WriteByte(ref writer, (byte)'=');
            WriteEncoded(ref writer, param.Value.AsSpan());
        }
    }
    ...
    private static void WriteAscii(ref DynamicMemory<byte> writer, ReadOnlySpan<char> text)
    {
    ...
    private static void WriteUtf8(ref DynamicMemory<byte> writer, ReadOnlySpan<char> text)
    {
    ...
    private static void WriteEncoded(ref DynamicMemory<byte> writer, ReadOnlySpan<char> text)
    {
        ...
        foreach (var rune in text.EnumerateRunes())
        {
            ...
            {
                WriteByte(ref writer, (byte)rune.Value);
            ...
            for (var i = 0; i < length; i++)
                WritePercentEncoded(ref writer, utf8[i]);
        }
    }

    private static void WriteByte(ref DynamicMemory<byte> writer, byte value)
    {
    ...
    private static void WritePercentEncoded(ref DynamicMemory<byte> writer, byte value)
    {
