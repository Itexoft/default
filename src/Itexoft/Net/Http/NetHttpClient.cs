...
public sealed class NetHttpClient : IDisposable
{
    ...
    private NetHttpResponse SendCore(NetHttpRequest request, CancelToken cancelToken)
    {
        ...
        if (!request.Timeout.IsInfinite && request.Timeout > TimeSpan.Zero)
            cancelToken = cancelToken.Branch(request.Timeout);

        return this.SendPrepared(request, cookieHeader, contentLength, secure, cancelToken, true);
    }

    private NetHttpResponse SendPrepared(
        NetHttpRequest request,
        string? cookieHeader,
        long? contentLength,
        bool secure,
        CancelToken cancelToken,
        bool allowReconnectRecovery)
    {
        var contentStartPosition = request.Content?.Position;
        ...
        try
        {
            ...
            var stream = this.lazyConnector.Connect(out reusedConnection, cancelToken);
        ...
        catch (Exception ex) when (allowReconnectRecovery
                                   && reusedConnection
                                   && request.ConnectionType == ConnectionType.KeepAlive
                                   && IsReconnectableKeepAliveFailure(ex)
                                   && (request.Content is null
                                       || (contentStartPosition is long rewindPosition
                                           && this.TryPrepareReconnectRetry(request.Content, rewindPosition))))
        {
            this.lazyConnector.Disconnect();

            return this.SendPrepared(request, cookieHeader, contentLength, secure, cancelToken, false);
        }
        catch (Exception ex)
        {
    ...
    private bool TryPrepareReconnectRetry(IStreamRs<byte>? content, long startPosition)
    {
        if (content is null)
            return true;
    ...
    private static bool IsReconnectableKeepAliveFailure(Exception ex) =>
        ex is NetHttpConnectionClosedException
        || ex is NetSocketException
        {
            SocketErrorCode: NetSocketError.ConnectionAborted
            or NetSocketError.ConnectionReset
