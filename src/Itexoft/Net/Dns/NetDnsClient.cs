...
using Itexoft.Optimization.Caching;
...
public class NetDnsClient : INetDnsResolver, IDisposable
{
    ...
    public NetIpAddress[] Resolve(NetHost host, CancelToken cancelToken = default)
    {
        ...
        return this.retryPolicy.WithTimeout(this.Timeout).Run(ct => this.ResolveCore(host, ct), cancelToken);
    }

    private protected virtual NetIpAddress[] ResolveCore(NetHost host, CancelToken cancelToken)
    {
        this.disposed.ThrowIf();
        cancelToken.ThrowIf();

        var addresses = this.cache.GetOrAdd(
            host,
            (h, ct) =>
            {
                var result = this.ResolveCore(h, ct, 0, true);

                if (result is not { Addresses: { Length: > 0 } resolved })
                    throw new NetSocketException(NetSocketError.HostNotFound);
    ...
    private IReadOnlyList<DnsRecordType> GetQueryTypes(NetAddressFamily addressFamily) => addressFamily switch
    {
        NetAddressFamily.InterNetwork => queryIPv4,
        NetAddressFamily.InterNetworkV6 => queryIPv6,
    ...
    private DnsResult? ResolveCore(NetHost host, CancelToken cancelToken, int depth, bool aggregateAll)
    {
        if (depth > 4)
            return null;

        var recordTypes = this.GetQueryTypes(host.AddressFamily);
        var orderedServers = this.SelectServers(NetStatus.Ipv4Available, NetStatus.Ipv6Available);
        NetSocketException? lastSocketException = null;
        ...
        foreach (var recordType in recordTypes)
        {
            foreach (var server in orderedServers)
            {
                ...
                try
                {
                    var response = this.QueryServer(server, host.Host, recordType, cancelToken);
                    ...
                    switch (response.Status)
                    {
                        ...
                        case DnsResponseStatus.Success when response.CanonicalName is { } cname:
                        {
                            ...
                            return this.ResolveCore(cnameKey, cancelToken, depth + 1, aggregateAll);
                        }
        ...
        if (lastResponse is { Status: DnsResponseStatus.TemporaryFailure })
            throw new NetSocketException(NetSocketError.TryAgain);

        if (lastSocketException is not null)
        {
            ExceptionDispatchInfo.Capture(lastSocketException).Throw();
    ...
    private DnsResponse QueryServer(NetIpEndpoint server, string host, DnsRecordType recordType, CancelToken cancelToken)
    {
        ...
        try
        {
            ...
            var udpResponse = this.QueryUdp(server, buffer, queryLength, messageId, recordType, cancelToken);
            ...
            return this.QueryTcp(server, buffer, queryLength, messageId, recordType, cancelToken);
        }
        finally
        {
        ...
        CancelToken cancelToken)
    {
        this.disposed.ThrowIf();
        cancelToken.ThrowIf();
        using var socket = new NetSocket(server.AddressFamily, NetSocketType.Dgram, NetProtocol.Udp);
        socket.SendTimeout = socket.ReceiveTimeout = this.Timeout;
        socket.Connect(server, cancelToken);
        socket.Send(new(buffer, 0, queryLength), NetSocketFlags.None, cancelToken);
        var received = socket.Receive(buffer, NetSocketFlags.None, cancelToken);
        ...
        CancelToken cancelToken)
    {
        this.disposed.ThrowIf();
        cancelToken.ThrowIf();
        using var socket = new NetSocket(server.AddressFamily, NetSocketType.Stream, NetProtocol.Tcp);
        socket.SendTimeout = socket.ReceiveTimeout = this.Timeout;
        socket.Connect(server, cancelToken);
        ...
        if (!sendAll(buffer, 0, queryLength))
            return DnsResponse.None;
        ...
        try
        {
            if (!receiveAll(receiveBuffer, 0, 2))
                return DnsResponse.None;
            ...
            if (!receiveAll(receiveBuffer, 0, responseLength))
                return DnsResponse.None;
        ...
        bool sendAll(byte[] data, int offset, int count)
        {
            var sent = 0;

            while (sent < count)
            {
                var written = socket.Send(new(data, offset + sent, count - sent), NetSocketFlags.None, cancelToken);
        ...
        bool receiveAll(byte[] data, int offset, int count)
        {
            var readTotal = 0;

            while (readTotal < count)
            {
                var read = socket.Receive(data.AsSpan(offset + readTotal, count - readTotal), NetSocketFlags.None, cancelToken);
    ...
    private List<NetIpEndpoint> SelectServers(bool ipv4Available, bool ipv6Available)
    {
        ...
        NetAddressFamily primary;
        NetAddressFamily secondary;
        var allowSecondary = true;

        switch (this.Preference)
        {
            case NetworkPreference.PreferIpv6:
                primary = NetAddressFamily.InterNetworkV6;
                secondary = NetAddressFamily.InterNetwork;

                break;
            case NetworkPreference.PreferIpv4:
                primary = NetAddressFamily.InterNetwork;
                secondary = NetAddressFamily.InterNetworkV6;

                break;
            case NetworkPreference.Ipv6Only:
                primary = NetAddressFamily.InterNetworkV6;
                secondary = NetAddressFamily.Unspecified;
    ...
    {
        if (addressFamily == NetAddressFamily.Unspecified)
            return;
    ...
    private static bool IsAvailable(NetAddressFamily addressFamily, bool ipv4Available, bool ipv6Available) =>
        addressFamily switch
        {
            NetAddressFamily.InterNetwork => ipv4Available,
            NetAddressFamily.InterNetworkV6 => ipv6Available,
            _ => false,
        };
