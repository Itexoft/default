...
public readonly struct NetIpAddress : ISpanFormattable, ISpanParsable<NetIpAddress>, IUtf8SpanFormattable, IUtf8SpanParsable<NetIpAddress>
{
    ...
    public NetIpAddress(ReadOnlySpan<byte> address, uint scopeId)
    {
        ...
        {
            case 4:
                this.AddressFamily = NetAddressFamily.InterNetwork;
                ...
                return;
            case 16:
                this.AddressFamily = NetAddressFamily.InterNetworkV6;
                ...
                return;
            default:
                this.AddressFamily = NetAddressFamily.Unspecified;
                this.block0 = 0;
                this.block1 = 0;
                this.scopeId = 0;
    ...
    public NetIpAddress(NetAddressFamily addressFamily, ulong block0, ulong block1, uint scopeId)
    {
    ...
    public NetAddressFamily AddressFamily { get; }

    public uint ScopeId => this.scopeId;

    public bool IsIPv4 => this.AddressFamily == NetAddressFamily.InterNetwork;

    public bool IsIPv6 => this.AddressFamily == NetAddressFamily.InterNetworkV6;

    public bool IsAny => this.block0 == 0 && this.block1 == 0 && this.scopeId == 0;

    public static NetIpAddress Any { get; } = IPAddress.Any;
    public static NetIpAddress IpV6Any { get; } = IPAddress.IPv6Any;
    ...
    public static implicit operator NetIpAddress(IPAddress address) =>
        address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetworkV6
            ? new(address.GetAddressBytes(), (uint)address.ScopeId)
    ...
    public int WriteBytes(Span<byte> destination)
    {
        ...
        {
            case NetAddressFamily.InterNetwork when destination.Length < 4:
                return 0;
            case NetAddressFamily.InterNetwork:
                Unsafe.WriteUnaligned(ref MemoryMarshal.GetReference(destination), (uint)this.block0);

                return 4;
            case NetAddressFamily.InterNetworkV6 when destination.Length < 16:
                return 0;
    ...
    private static bool TryParseIpv4(ReadOnlySpan<char> text, out NetIpAddress address)
    {
        ...
        b3 = (byte)octet;
        address = new(NetAddressFamily.InterNetwork, PackIPv4(b0, b1, b2, b3), 0, 0);
    ...
    private static bool TryParseIpv6(ReadOnlySpan<char> text, uint scopeId, out NetIpAddress address)
    {
        ...
        address = new(NetAddressFamily.InterNetworkV6, b0, b1, scopeId);
    ...
    public bool TryFormat(Span<char> destination, out int charsWritten, ReadOnlySpan<char> format)
    {
        ...
        {
            NetAddressFamily.InterNetwork => this.TryFormatIpv4(destination, out charsWritten),
            NetAddressFamily.InterNetworkV6 => this.TryFormatIpv6(destination, out charsWritten),
            _ => false,
        };
    }
    ...
    {
        if (ipAddress.AddressFamily == NetAddressFamily.InterNetwork
            || (ipAddress.AddressFamily != NetAddressFamily.InterNetworkV6 && ipAddress.block1 == 0 && ipAddress.scopeId == 0))
        {
    ...
    {
        if (this.AddressFamily == NetAddressFamily.InterNetwork
            || (this.AddressFamily != NetAddressFamily.InterNetworkV6 && this.block1 == 0 && this.scopeId == 0))
        {
            Span<byte> bytes = stackalloc byte[4];
