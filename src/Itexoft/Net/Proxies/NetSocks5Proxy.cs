...
public readonly struct NetSocks5Proxy(NetEndpoint endpoint, NetCredential credential = default) : INetProxy
{
    ...
    public NetSocks5Proxy(NetHost host, NetPort port, NetCredential credential = default, NetProtocol protocol = NetProtocol.Tcp) : this(
        new(host, port, protocol),
        credential) => this.protocol = protocol;
    ...
    {
        if (endpoint.TryCreate(out var ip, this.protocol))
        {
