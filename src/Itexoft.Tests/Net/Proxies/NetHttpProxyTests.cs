...
public sealed class NetHttpProxyTests
{
    [Test]
    public async Task ConnectAsync_SendsConnectWithAuth()
    {
        ...
        var proxy = new NetHttpProxy("127.0.0.1", server.EndPoint.Port, new("user", "pass"));
    ...
    [Test]
    public void ConnectAsync_ThrowsOnNonSuccessStatus()
    {
        ...
        Assert.That(
            async () =>
            {
                var server = await ProxyTestServers.StartHttpAsync(_ => response).ConfigureAwait(false);
                await using var server1 = server.ConfigureAwait(false);
                var proxy = new NetHttpProxy("127.0.0.1", server.EndPoint.Port);
                await new NetTcpConnector(proxy).ConnectAsync(("example.com", 443), CancelToken.None).ConfigureAwait(false);
            },
